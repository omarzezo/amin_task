...
class GetHomeData extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<GetDataProvider>(
        builder: (context, state, child) {print('${state.depositeModel?.status}');
      return Scaffold(
        appBar: AppBar(
            ...
            children: [
             ...
             if(state.data.isNotEmpty) Expanded(
                  child: state.status == Status.LOADING ? const Center(child: CircularProgressIndicator()) : ListView.builder(
                      itemCount:state.data.values.first.length,
                    // itemCount: 5,
                      itemBuilder: (context, index) {
                        return Padding(
                          ...
                          child: ListTile(
                            title: Text(
                                '${state.data.values.first[index].date}'),
                            subtitle: Text(
                                '${state.data.values.first[index].time}'),
